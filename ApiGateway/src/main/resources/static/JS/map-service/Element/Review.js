...
export function setReviewData(data, myUserId){
        ...
        data.forEach(reviewData => {
                ...
                if(myUserId === reviewData.writer)
                        reviewerName.textContent = "내 리뷰";
                else
                ...
                scoreElement.textContent = reviewData.score+"/5 점";
                ...
                if (reviewData.writer === myUserId) {
                        // 해당 리뷰를 가장 위로 올리기
                        container.insertBefore(reviewBox, container.firstChild);
                } else {
                        // 그 외 리뷰는 기본적으로 추가
